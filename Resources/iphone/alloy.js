(function(tirequire,__dirname,__filename){module.id=__filename;module.loaded=false;module.filename=__filename;var require=tirequire("node_modules/ti-commonjs/lib/ti-commonjs")(__dirname,module);module.require=require;function ucfirst(text) {
    if (!text) return text;
...
var _ = tirequire("alloy/underscore")._, Backbone = tirequire("alloy/backbone"), CONST = tirequire("alloy/constants");
...
exports.M = function(name, modelDesc, migrations) {
    ...
    var mod;
    if (adapter.type) {
        mod = tirequire("alloy/sync/" + adapter.type);
...
exports.C = function(name, modelDesc, model) {
    ...
    if (config.adapter && config.adapter.type) {
        mod = tirequire("alloy/sync/" + config.adapter.type);
        extendObj.sync = function(method, model, opts) {
            mod.sync(method, model, opts);
        };
    } else extendObj.sync = function(method, model) {
...
exports.createStyle = function(controller, opts, defaults) {
    ...
    var styleArray;
    styleArray = controller && _.isObject(controller) ? tirequire("alloy/widgets/" + controller.widgetId + "/styles/" + controller.name) : tirequire("alloy/styles/" + controller);
    var styleFinal = {};
    var i, len;
...
exports.createWidget = function(id, name, args) {
    ...
    return new (tirequire("alloy/widgets/" + id + "/controllers/" + (name || DEFAULT_WIDGET)))(args);
};

exports.createController = function(name, args) {
    return new (tirequire("alloy/controllers/" + name))(args);
};

exports.createModel = function(name, args) {
    return new (tirequire("alloy/models/" + ucfirst(name)).Model)(args);
};

exports.createCollection = function(name, args) {
    return new (tirequire("alloy/models/" + ucfirst(name)).Collection)(args);
};
