(function(tirequire,__dirname,__filename){module.id=__filename;module.loaded=false;module.filename=__filename;var require=tirequire("node_modules/ti-commonjs/lib/ti-commonjs")(__dirname,module);module.require=require;var Alloy = tirequire('alloy');
...
module.exports = function(widgetId) {
	...
	this.createController = function(name, args) {
		return new (tirequire('alloy/widgets/' + widgetId + '/controllers/' + name))(args);
	};
	this.createCollection = function(name, args) {
		return new (tirequire('alloy/widgets/' + widgetId + '/models/' + ucfirst(name)).Collection)(args);
	};
	this.createModel = function(name, args) {
		return new (tirequire('alloy/widgets/' + widgetId + '/models/' + ucfirst(name)).Model)(args);
	};
...
module.loaded=true;})(require,"/alloy","/alloy/widget.js");